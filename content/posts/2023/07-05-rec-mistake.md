...
答も合わないし，そもそも実行するごとに異なる値になる現象があった．
未初期値など使っていないことは目視で確かめたので，なにがなんだかしばらくわからなかった．
...
mp[x] のところで，mp にキー x が追加されるが，「値 0 が追加されても困らないし，
実行時間制限には十分余裕があるし」と思って放っておいたのが失敗．
mp に関するループなのだから，ループ内でキーを追加してしまっては何が起こるか分からない．
ループ内で同じ要素に複数回アクセスすることが起こって答が合わなかった．
また，myhash 関数内で乱数を使っているので，実行ごとに異なる値になった．
