...
こちらは，2つのクエリ (merge, leader) が混ざってきても良い．
...
すると，ある時点で $a_k + i$ と $b_k + i$ が merge 前にすでに同じ連結成分に属していたら，
$j \geq i$ に対する $a_k + j$ と $b_k + j$ の組は見る必要が無い．
$a_k + i$ と $b_k + i$ を同じ連結成分にするために必要だったそれぞれの merge(x, y) に対し，
merge(x + (j - i), y + (j - i)) も実行されるからである．
